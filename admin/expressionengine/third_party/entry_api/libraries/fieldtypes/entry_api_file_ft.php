...
class Entry_api_file_ft
{
	...
	protected function do_upload_file($post_data = array(), $field)
	{
		...
		{		
			...
			{
				$this->service_error['error_file']['message'] = str_replace(array('<p>', '</p>'), '',$file_dir['error']);
				return $this->service_error['error_file'];
			}
	...
	public function upload_file($file_data = '', $filename = '', $dir_id = '', $field_settings)
	{
		...
		if(empty($file_data) || $file_data == '')
		{
			...
			return array(
				'code' => $this->service_error['error_no_file_data_given_no'], 
				'message' => $this->service_error['error_no_file_data_given'],
			);
		}
		...
		if(empty($filename) || $filename == '' )
		{
			...
			return array(
				'code' => $this->service_error['error_no_filename_given_no'], 
				'message' => $this->service_error['error_no_filename_given'], 
			);
		}	
		...
		if($field_settings['field_settings']['allowed_directories'] == 'all')
		{
			...
			if($dir_id == '') {
				//return error, no filename
				return array(
					'code' => $this->service_error['error_no_dir_id_given_no'], 
					'message' => $this->service_error['error_no_dir_id_given'], 
				);
			}
		...
		if(!@file_put_contents(ee()->entry_api_settings->item('tmp_dir').$filename, $file_data))
		{
			//return error, can`t save the file to the tmp map
			return array(
				'code' => $this->service_error['error_cannot_save_tmp_file_no'],
				'message' => $this->service_error['error_cannot_save_tmp_file'] . ee()->entry_api_settings->item('tmp_dir').$filename,
			);
		}
		...
		//get the file info
		$file_info = get_file_info(ee()->entry_api_settings->item('tmp_dir').$filename);
		
		//build a file array for the file
		$file['field_id_'.$field_settings['field_id']] = array(
			'name'     => $filename,
			'type'     => get_mime_by_extension(ee()->entry_api_settings->item('tmp_dir').$filename),
			'tmp_name' => ee()->entry_api_settings->item('tmp_dir').$filename,
			'size'     => $file_info['size'],
		);
	...
	{		
		...
		ee()->load->library('Entry_api_upload_manually');
		ee()->entry_api_upload_manually->initialize($config);
		
		if ( ! ee()->entry_api_upload_manually->do_upload_manually($file))
		{
			//remove the tmp file
			@unlink($file['tmp_name']);
			
			return ee()->filemanager->_upload_error(
				ee()->entry_api_upload_manually->display_errors()
			);
		}
		
		//get the file info
		$fileinfo = ee()->entry_api_upload_manually->data();
