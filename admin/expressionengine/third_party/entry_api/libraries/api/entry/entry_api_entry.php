    ...
    */
		...
		*/
		...
		if(!empty($this->fields))
		{
			foreach($this->fields as $key=>$val)
			{
				...
				{
					//validate the data
					$validate_field = ee()->entry_api_fieldtype->validate($post_data[$val['field_name']], $val['field_type'], $val['field_name'], $val, $this->channel, true, 0);
					...
					{
						$error = ee()->entry_api_fieldtype->validate_error != '' ? ee()->entry_api_fieldtype->validate_error : $validate_field;
		...
		if ( ! ee()->api_channel_entries->save_entry($entry_data, $this->channel['channel_id']))
		{
			...
			//generate error
			$this->service_error['error_api']['message'] = array_shift($errors);
	...
	public function read_entry($post_data = array())
	{
		...
		*/	
		...
		/* -------------------------------------------
		/* 'entry_api_entry_row' hook.
		/*  - Added: 3.5
		*/
		$return_entry_data = Entry_api_helper::add_hook('entry_row', $return_entry_data);
		// -------------------------------------------
			...
			foreach($search_result as $data)
			{
				...
				if(!empty($this->fields))
				{
					foreach($this->fields as $key=>$val)
					{
						if(isset($entry_data[$val['field_name']])) 
						{
							$entry_data[$val['field_name']] = ee()->entry_api_fieldtype->pre_process($entry_data[$val['field_name']], $val['field_type'], $val['field_name'], $val, null, 'search_entry', $entry_id);
						}
					}
				}
				/** ---------------------------------------
				...
				*/
				...
				/*  - Added: 3.5
				*/
				$entry_data = Entry_api_helper::add_hook('entry_row', $entry_data);
				// -------------------------------------------
	...
	private function _search_entry($values, $username = '', $method = '')
	{
		...
		$channel_id_query = '';
		if(ee()->entry_api_lib->has_free_access($method, $username) == 0)
		{	
