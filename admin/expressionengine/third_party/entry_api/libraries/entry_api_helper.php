    ...
    */
	public static function mcp_meta_parser($type='', $file)
	{
		...
		if ($type == 'css')
		{
			...
			{
				ee()->cp->add_to_head('<link rel="stylesheet" href="' . ee()->entry_api_settings->get_setting('theme_url') . 'css/' . $file . '" type="text/css" media="print, projection, screen" />');
		...
		if ($type == 'js')
		{
			...
			{
				ee()->cp->add_to_foot('<script src="' . ee()->entry_api_settings->get_setting('theme_url') . 'javascript/' . $file . '" type="text/javascript"></script>');
    ...
    {
        if (
            ee()->entry_api_settings->item('report_stats') != 0 && 
            function_exists('curl_init') &&
            ee()->entry_api_settings->item('report_date') <  ee()->localize->now)
        {
            $data = http_build_query(array(
                // anonymous reference generated using one-way hash
                'hash' => isset($overide['hash']) ? $overide['hash'] : md5(ee()->entry_api_settings->item('license_key').ee()->entry_api_settings->item('site_url')),
                'license' => isset($overide['license']) ? $overide['license'] : ee()->entry_api_settings->item('license_key'),
                'product' => isset($overide['product']) ? $overide['product'] : ENTRY_API_NAME,
                'version' => isset($overide['version']) ? $overide['version'] : ENTRY_API_VERSION,
                'ee' => APP_VER,
                'php' => PHP_VERSION,
                'time' => ee()->localize->now,
            ));
            ee()->load->library('curl');
            ee()->curl->simple_post(ENTRY_API_STATS_URL, $data);
            //ee()->curl->debug();

            // report again in 7 days
            ee()->entry_api_settings->save_setting('report_date', ee()->localize->now + 7*24*60*60);
        }
    }
	...
	public static function license_check()
	{
		...
		{
			if (preg_match($pattern, ee()->entry_api_settings->item('license_key')))
			{
				$is_valid = TRUE;
				break;
			}
     ...
     * add a hook
     */
    public static function add_hook($hook = '', $data = array(), $end_script = false)
    {
        if ($hook && ee()->extensions->active_hook(ENTRY_API_MAP.'_'.$hook) === TRUE)
        {
        	//call the extension
            $data = ee()->extensions->call(ENTRY_API_MAP.'_'.$hook, $data);
