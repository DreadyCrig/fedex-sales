...
{	
	...
	*/
	...
	public function default_checks($auth = array(), $method = '', $site_id = 1)
	{
		...
		/**  No auth data
		/** ---------------------------------------*/
		if(empty($auth) && ee()->entry_api_lib->has_free_access($method, ee()->session->userdata('username')) == 0)
		{	
			//generate error
			return array(
				'succes' => false,
	 ...
	 * @return void           
	 */
	public function auth($auth = array())
	{
		...
		{
			$res = $this->auth_data = $this->base_authenticate_username($auth['username'], $auth['password']);
			//is response false?
			if($res['code_http'] == '503')
			{
				$this->error_str = $res['message'];
				return false;
			}
