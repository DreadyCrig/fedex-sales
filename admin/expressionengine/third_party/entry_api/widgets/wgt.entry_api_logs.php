...
class Wgt_entry_api_logs
{
	...
	{	
		...
		ee()->lang->loadfile('entry_api');

		$this->settings = array(
			'title' => lang('wgt_entry_api_logs_title'),
			'nr' => 5
		);
		$this->title  	= lang('wgt_entry_api_logs_title');
	...
	public function index($settings = NULL)
	{
		...
		//get the logs
		ee()->db->select('*');
		ee()->db->from('entry_api_logs');
		ee()->db->where('site_id', ee()->config->item('site_id'));
		ee()->db->where('method', 'Entry_api_service::create_entry');
		ee()->db->or_where('method', 'Entry_api_service::update_entry');
		ee()->db->or_where('method', 'Entry_api_service::delete_entry');
